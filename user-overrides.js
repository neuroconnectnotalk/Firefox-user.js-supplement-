...
user_pref("security.ask_for_password", 1);                          // [PRIV=1] when to ask for the master password - 0=the first time it's needed, 1=every time it's needed, 2=every n minutes where n is the value of security.password_lifetime.
...
user_pref("browser.sessionstore.max_tabs_undo", 99);                 // how many tabs that can be restored when restarting the browser if session restore is enabled, as well as how many closed tabs can be re-opened while browsing
...
user_pref("extensions.pocket.enabled", true);                      // [SET] [*PRIV=false] set to 'true' if you use the Pocket service, a "save for later" cloud service
...
user_pref("media.autoplay.default", 0);                             // [SET] media playback - 0=allow all, 1=block non-muted, 2=prompt (removed in FF66), 5=block all (FF69+)
 ...
 *
 * https://github.com/arkenfox/user.js/issues/123
 *
